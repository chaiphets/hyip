...
class Raw_material_model extends CI_Model {
	function createMaterial($material){
		...
		$query = $this->db->get('fg_raw_material');
		$newId = 1;
		$last = $query->first_row('array');
		if($last)
			$newId = intval($last['material_id']) + 1;
		
		//fg_raw_material_files
		if(isset($material['files'])){
			foreach($material['files'] as $key => $files){
				$file['material_id'] = $newId;
				$file['file_id'] = $key + 1;
				$file['file_name'] = $files['file_name'];
				$file['full_path'] = 'img/uploads/raw_material/'.sprintf('%04d',$file['material_id']).'_'.$file['file_name'];
				$this->db->insert('fg_raw_material_files', $file);
		...
		$this->db->insert('fg_raw_material', $material);
...
// 			
	...
	function getMaterial($materialId){
		$this->db->join('fg_user', 'fg_raw_material.owner_user_id = fg_user.user_id');
		$query = $this->db->get_where('fg_raw_material', array('material_id'=>$materialId));
		$material = $query->row_array();
		
		$material['purchased_date'] = date('d/m/Y', strtotime($material['purchased_date']));
		
		$query = $this->db->get_where('fg_raw_material_files', array('material_id'=>$materialId));
		...
		// task list
		$this->load->model('task/task_model');
		$material['tasks'] = $this->task_model->getTaskDetailByTypeAndTypeId('fg_raw_material',$materialId);
