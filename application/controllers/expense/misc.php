...
class Misc extends CI_Controller{
	public function __construct(){
		...
		$this->load->model('expense/misc_model');
	}
	...
	public function index(){
		$data['materials'] = $this->misc_model->getAllMaterial();
		$this->template->load('expense/misc/material_list', $data);
	}
	...
	public function showMaterial($materialId=null){
		...
		$material = $this->misc_model->getMaterial($materialId);
	...
	public function saveMaterial(){
		...
		$this->form_validation->set_rules('claim_rate', 'Claim Rate', 'decimal|less_than[1.01]');
			...
			if(strlen($file_name) > 0){
				...
				foreach($material['files'] as $key => $file){
					if($file['file_name'] == $file_name){
						$this->template->load('expense/misc/material_detail', array('material'=>$material, 'upload_error'=>$upload_error));
						return;
					}
				}
		...
		$this->misc_model->createMaterial($material);
