...
class Material extends CI_Controller{
	public function __construct(){
		...
		$this->load->model('expense/material_model');
	}
	private function __initMaterial(){
		$material['material_name'] = null;
		$material['material_brand'] = null;
		$material['material_serial'] = null;
		$material['material_price'] = null;
		$material['purchased_date'] = null;
		$material['claim_rate'] = 1.0;
	...
	public function index(){
		$data['materials'] = $this->material_model->getAllMaterial();
		$this->template->load('expense/material/material_list', $data);
	}
	...
	public function showMaterial($materialId=null){
		...
		$material = $this->material_model->getMaterial($materialId);
	...
	public function saveMaterial(){
		...
		$this->form_validation->set_rules('material_brand', 'Material Brand', 'min_length[3]');
		$this->form_validation->set_rules('material_serial', 'Material Serial', 'min_length[3]');
			...
			if(strlen($file_name) > 0){
				...
				foreach($material['files'] as $key => $file){
					if($file['file_name'] == $file_name){
						$this->template->load('expense/material/material_detail', array('material'=>$material, 'upload_error'=>$upload_error));
						return;
					}
				}
		...
		$this->material_model->createMaterial($material);
